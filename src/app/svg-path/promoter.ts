import { CurveTo, DrawTo, isHLineTo, isLineTo, isQCurveTo, isSmoothCurveTo, isSmoothQCurveTo, isVLineTo, QCurveTo } from './command';
...
export function promoteToQCurve(item: Readonly<PathNode>): QCurveTo {
  if (isSmoothQCurveTo(item)) {
    const x1 = getReflectedX1(item);
    const y1 = getReflectedY1(item);
    return { name: 'Q', x1, y1, x: item.x, y: item.y };
  } else {
    // Treat it as a line.
    ...
    const x1 = (x0 + x) / 2;
    const y1 = (y0 + y) / 2;
    return { name: 'Q', x1, y1, x, y };
  }
}

export function promoteToCurve(item: Readonly<PathNode>): CurveTo {
  if (isSmoothCurveTo(item)) {
    const x1 = getReflectedX1(item);
    const y1 = getReflectedY1(item);
    return { name: 'C', x1, y1, x2: item.x2, y2: item.y2, x: item.x, y: item.y };
  } else {
    const x0 = getX(item.prev);
    const y0 = getY(item.prev);
    if (isQCurveTo(item) || isSmoothQCurveTo(item)) {
      ...
      return { name: 'C', x1, y1, x2, y2, x: item.x, y: item.y };
    } else {
      // Treat it as a line.
      const x = getX(item);
      const y = getY(item);
