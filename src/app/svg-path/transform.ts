...
import { EllipseShape, isClosePath, isCurveTo, isEllipticalArc, isHLineTo, isQCurveTo, isSmoothCurveTo, isVLineTo } from './command';
import { getX, getY, PathNode } from './node';

export function transformedX(m: ReadonlyMatrix, x: number, y: number) {
  return m.a * x + m.c * y + m.e;
}

export function transformedY(m: ReadonlyMatrix, x: number, y: number) {
...
export function transformedEllipse(m: ReadonlyMatrix, ellipse: Readonly<EllipseShape>): EllipseShape {
  ...
  const rx_ = Math.abs(D_) /
    Math.sqrt(A_ * cosPhi_ * cosPhi_ + B_ * sinPhi_ * cosPhi_ + C_ * sinPhi_ * sinPhi_);
  const ry_ = Math.abs(D_) /
    Math.sqrt(A_ * sinPhi_ * sinPhi_ - B_ * sinPhi_ * cosPhi_ + C_ * cosPhi_ * cosPhi_);
